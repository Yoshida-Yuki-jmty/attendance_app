...
RSpec.describe Attendance, type: :model do
  describe "validations" do
    it "同一ユーザーで work_date の重複を許可しない" do
      u = create(:user)
      create(:attendance, user: u, work_date: Date.new(2025,9,1))
      dup = build(:attendance, user: u, work_date: Date.new(2025,9,1))
      expect(dup).to be_invalid
      expect(dup.errors[:work_date]).to be_present
    end
  ...
  describe ".clock_in!" do
    ...
    it "レコードが無ければ作成する" do
      travel_to Time.zone.parse("2025-09-08 09:00") do
        ...
        expect(a.work_date).to eq Date.new(2025,9,8)
